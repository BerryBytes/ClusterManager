"""
Module to update cluster status and simulate cluster lifecycle via API.

Provides utility functions to send status updates and simulate a cluster's
lifecycle for testing purposes.
"""
...
def update_cluster_status(cluster_id, status, server_url="http://localhost:8000"):
    ...
    payload = {"id": cluster_id, "status": status}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Update cluster status or simulate lifecycle"
    )
    ...
    parser.add_argument(
        "--server", "-u", default="http://localhost:8000", help="API server URL"
    )
    parser.add_argument(
        "--simulate",
        "-sim",
        action="store_true",
        help="Simulate full cluster lifecycle",
    )
