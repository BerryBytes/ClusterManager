...
def list_user(request: Request):
    try:
        ...
        return generate_response(False, status.HTTP_500_INTERNAL_SERVER_ERROR, "Failed to list users", [])

@router.get("/subscription-check", response_description="subscription check", status_code=status.HTTP_202_ACCEPTED, response_model=ResponseModel)
def subscription_check(request: Request): 
    try:
        ...
        return generate_response(True, status.HTTP_202_ACCEPTED, "Subscription check successful", response)
    except Exception as e:
        debug_response(e, "Error occurs on subscription check", "error")
        return generate_response(False, status.HTTP_400_BAD_REQUEST, "Failed to check subscription", [])

@router.get("/{id}", response_description="Get a single user by id",response_model=ResponseModel)
def find_user(id: str, request: Request):
    try:
        ...
        return generate_response(False, status.HTTP_404_NOT_FOUND, f"User with ID {id} not found", [])
    except Exception as e:
        debug_response(e, "Error occurs on retrieving user", "error")
        return generate_response(False, status.HTTP_500_INTERNAL_SERVER_ERROR, "Failed to retrieve user", [])

@router.post("/ping", response_description="verified_user", status_code=status.HTTP_202_ACCEPTED)
def ping_user():
    return generate_response(True, status.HTTP_202_ACCEPTED, "Ping successful", [])

@router.post("/verify-token", response_description="verified_user", status_code=status.HTTP_202_ACCEPTED, response_model=ResponseModel)
def token_verify(request: Request):
    try:
        ...
        return generate_response(False, status.HTTP_500_INTERNAL_SERVER_ERROR, "Failed to verify token", [])

@router.post("/subscription-request", response_description="request for subscription", status_code=status.HTTP_202_ACCEPTED,response_model=ResponseModel)
def subscription_request(request: Request):    
    try:
        ...
        if not user_groups:  # If user_groups is empty
            group_id = keycloak_admin.get_group_by_path(path="/" + REQUEST_GROUP_NAME).get("id")
            ...
            return generate_response(False, status.HTTP_400_BAD_REQUEST, "User already requested for subscription", [])
        else:
            return generate_response(False, status.HTTP_400_BAD_REQUEST, "User already approved for subscription", [])

        response = {
            "status": status_code,
            "message": message
        }
        return generate_response(True, status.HTTP_202_ACCEPTED, "Subscription request sent successfully", [response])
    except Exception as e:
        debug_response(e, "Error occurs on subscription request", "error")
        return generate_response(False, status.HTTP_500_INTERNAL_SERVER_ERROR, "Failed to request subscription", [])