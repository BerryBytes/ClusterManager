"""
Cluster Subscription, Status, and Login API Routes.

Provides endpoints to list subscriptions, update cluster status with WebSocket
notifications, and perform user login via Keycloak.
"""
...
from routes.websocket import (  # Import both functions
    broadcast_message,
...
@router.patch(
    "/cluster-status",
    response_description="Post pod Status",
    status_code=status.HTTP_200_OK,
)
async def update_cluster_status(
    request: Request, data: ClusterStatusRequest = Body(...)
):
    ...
    try:
        ...
        if "user_id" in cluster:
            ...
            "user" in cluster
            and isinstance(cluster["user"], dict)
        ...
        if user_id:
            ...
            print(
                f"WebSocket notification {'sent' if notification_sent else 'not sent'} to user {user_id}"
            )
